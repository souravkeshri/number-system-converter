...
public class BinaryActivity extends AppCompatActivity {
    ...
    public void numberPressed(View v)
    {
        ...
        textViewBinary.setText(textViewBinary.getText().toString()+input);

    }
    public void clearBtn(View v)
    {
        textViewBinary.setText("");
    ...
    {
        String str=textViewBinary.getText().toString();
        if(str!=null && str.length()>1) {
            textViewBinary.setText(str.substring(0, str.length() - 1));
    ...
    {
        String exp=textViewBinary.getText().toString();
        if(exp.equals(""))
        {
            textViewBinary.setText("");
            textViewOctal.setText("Octal :                        ");
            ...
            for (int i = 0; i < operandList.size(); i++) {
                operandsInDecimal.add(i, Integer.toString(Integer.parseInt(operandList.get(i), 2), 10));
            }

            ArrayList<String> dexpression = new ArrayList<String>(operandsInDecimal.size());
            if (operandsInDecimal.size() != (operatorList.size() + 1)) {
                ...
                for (int i = 0, j = 0, k = 0; i < operandsInDecimal.size() + operatorList.size(); i++) {
                    if (i % 2 == 0)
                        dexpression.add(operandsInDecimal.get(j++));
                    else
                        dexpression.add(operatorList.get(k++));
                }

                StringBuilder myStringA = new StringBuilder();
                String ExpressionDecimal = new String();
                for (String i : dexpression) {
                    myStringA.append(i);
                }
                ...
                textViewQuestion.setText(textViewBinary.getText().toString());
                textViewQuestion.setVisibility(View.VISIBLE);

                textViewBinary.setText(Integer.toString(Integer.parseInt(DecimalResult, 10), 2));
    ...
    {
        Intent intent=new Intent(BinaryActivity.this,MainActivity.class);
